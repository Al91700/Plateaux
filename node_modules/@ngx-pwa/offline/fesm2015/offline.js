import { Inject, Injectable, InjectionToken, PLATFORM_ID, defineInjectable, inject } from '@angular/core';
...
class Network {
    ...
    initOnlineObservable() {
        this.onlineChanges = !isPlatformBrowser(this.platformId) ? of(true) : merge(startWith(this.online), fromEvent(document, 'online').pipe(mapTo(true)), fromEvent(document, 'offline').pipe(mapTo(false)));
    }
}
...
/** @nocollapse */ Network.ngInjectableDef = defineInjectable({ factory: function Network_Factory() { return new Network(inject(Router), inject(PLATFORM_ID), inject(OFFLINE_ROUTE_OFFLINE), inject(OFFLINE_ROUTE_UNAVAILABLE)); }, token: Network, providedIn: "root" });
/**
